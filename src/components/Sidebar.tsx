...
import { LayoutDashboard, Users, Activity, BarChart3, Calendar, FileText, Target, Award, Video, Moon, Sun, Menu, X, Shield, Brain, BookOpen, Layers, Radar, Film } from 'lucide-react';
  ...
  const inner = (
    <>
        ...
        {allNav.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)}
              className={cn('flex items-center gap-2 px-3 py-1.5 rounded-lg text-[13px] font-semibold transition',
      ...
      <div className="px-3 py-2 text-[10px] text-center border-t" style={{ borderColor: 'var(--border)', color: 'var(--text-secondary)' }}>
        <span className="font-bold" style={{ color: 'var(--text-primary)' }}>Ajman Coach Pro</span> v4
